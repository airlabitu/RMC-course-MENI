  ...
  if (cam.available() == true) {
    ...
    for( FiducialFound fiducial : found ) {
      ...
      //if (!QRObjects.containsKey(id)) QRObjects.put(qr.message, new QRObject(int(qr.message))); // add qr object if not already existing
        ...
        if (!QR_obj.isActive) {
          ...
          if (sendMIDI) {
            //myBus.sendControllerChange(0, id+3, 127); // send midi data parameter order (channel, number, value)
            
            OscMessage myMessage = new OscMessage("/on_off/");
            myMessage.add(0); /* add an int to the osc message */
            myMessage.add(id+3); /* add an int to the osc message */
        ...
        if (sendMIDI){
          ...
          OscMessage myMessage = new OscMessage("/trackingCenterRotation/");
          myMessage.add(0); /* add an int to the osc message */
          myMessage.add(id); /* add an int to the osc message */
          myMessage.add(QR_obj.trackingCenterRotation); /* add an int to the osc message */
          oscP5.send(myMessage, myRemoteLocation); 
          ...
          myMessage = new OscMessage("/trackingCenterDistance/");
          myMessage.add(0); /* add an int to the osc message */
          myMessage.add(id+1); /* add an int to the osc message */
          myMessage.add(QR_obj.trackingCenterDistance); /* add an int to the osc message */
          oscP5.send(myMessage, myRemoteLocation); 
          ...
          myMessage = new OscMessage("/midiRotationVal/");
          myMessage.add(0); /* add an int to the osc message */
          myMessage.add(id+2); /* add an int to the osc message */
          myMessage.add(QR_obj.midiRotationVal); /* add an int to the osc message */
          oscP5.send(myMessage, myRemoteLocation);
    ...
    for (Map.Entry me : QRObjects.entrySet()) {
      ...
      if (QR_obj.framesSinceActive > 60 && QR_obj.isActive == true) {
        ...
        if (sendMIDI) {
          //myBus.sendControllerChange(0, QR_obj.id+3, 0); // send midi data parameter order (channel, number, value)
          OscMessage myMessage = new OscMessage("/on_off/");
          myMessage.add(0); /* add an int to the osc message */
          myMessage.add(QR_obj.id+3); /* add an int to the osc message */
          myMessage.add(0); /* add an int to the osc message */
          oscP5.send(myMessage, myRemoteLocation); 
        }
      }
      ...
      QR_obj.updateFade();
      if (sendMIDI) {
        //myBus.sendControllerChange(0, QR_obj.id+4, int(QR_obj.isActiveFade)); // send midi data parameter order (channel, number, value)
      //println(QR_obj.id, QR_obj.isActive, QR_obj.framesSinceActive, QR_obj.isActiveFade);
        OscMessage myMessage = new OscMessage("/on_off_fade/");
        myMessage.add(0); /* add an int to the osc message */
        myMessage.add(QR_obj.id+4); /* add an int to the osc message */
        myMessage.add(int(QR_obj.isActiveFade)); /* add an int to the osc message */
        oscP5.send(myMessage, myRemoteLocation); 
      }
    }
